...
namespace Convolutional.Logic.Interleaver
{
    ...
    {
        private readonly IReadOnlyList<int> indicesInterleave;
        private readonly IReadOnlyList<int> indicesDeinterleave;
        ...
        {
            this.indicesInterleave = indicesInterleave.ToArray();
            blockSize = this.indicesInterleave.Count;
            
            if (this.indicesInterleave.Any(i => i >= blockSize) || this.indicesInterleave.NonUnique().Any())
            ...
            var reverse = new int[this.blockSize];
            for (var i = 0; i < reverse.Length; i++)
                reverse[this.indicesInterleave[i]] = i;
